...
$$
\begin{align*}
E_{sc}&=\mathrm{CONV}_{2D}[3](X_s),\\
E_{sh}&=\{\mathrm{CONV}_{2D}[2^i + 1](X_s)\},i \in \{2,3,4\},\\
E_{tc}&=\mathrm{ATTENTION}(X_c),\\
E_{tp}&=\mathrm{ATTENTION}(X_p).
\end{align*}
$$


$E_{s c}$ 是通过对 $X_{s}$ 进行卷积核大小为3的二维卷积运算得到的。  
$E_{s h}$ 是由对 $X_{s}$ 进行不同卷积核大小 $2^{i}+1$，其中 $i$ 取值为 2、3、4 的二维卷积运算的结果组成的集合。  
 $E_{t c}$ 是对 $X_{c}$ 进行注意力运算得到的结果。  
$E_{t p}$ 是对 $X_{p}$ 进行注意力运算，从而聚合周期性序列得到的结果。 
## 2.2 模型结构
