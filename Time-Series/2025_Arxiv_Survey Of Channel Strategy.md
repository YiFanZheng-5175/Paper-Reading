...
这些方法都采用了跨维度（CD）策略。将时间序列片段（补丁）或每个通道的整个序列视为单独的token，并直接应用注意力机制对通道相关性进行建模。例如，CARD [Wang 等人，2024b]和 iTransformer [Liu 等人，2024b]分别将每个通道的补丁和序列表示为独立的标记，并使用注意力机制明确捕获通道相关性。
